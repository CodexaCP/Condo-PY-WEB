            ...
            <div class="form-row">
              ...
              <label class="field-block">
                ...
                <select [(ngModel)]="recurringForm.distributionType" name="recDistribution" required>
                  <option *ngFor="let d of distributionTypes" [value]="d" [disabled]="!recurringForm.buildingId && d === 'IndividualUnit'">{{ distributionTypeLabel(d) }}</option>
                </select>
              </label>
              <label class="field-block">
                <span>Activa</span>
...
export class BuildingExpensesPageComponent implements OnInit {
  ...
  submitRecurring(): void {
    // buildingId vacio = plantilla general (sin edificio), aplicable a todos.
    const request: RecurringBuildingExpenseUpsertRequest = {
      buildingId: this.recurringForm.buildingId || null,
